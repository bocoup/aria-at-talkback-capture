...
add_to_path() {
  ...
  if [[ "$path_added" == "true" ]]; then
    echo "Please run 'source ~/.bashrc' or 'source ~/.zshrc' (depending on your shell) or restart your terminal to apply changes"
  else
...
download_adb() {
  ...
  local temp_dir="/tmp/platform-tools-temp"
  # Remove existing directory if it exists
  rm -rf "$temp_dir"
  mkdir -p "$temp_dir"

  # Download platform-tools
  curl -L "https://dl.google.com/android/repository/platform-tools-latest-linux.zip" -o "$temp_dir/platform-tools.zip"
